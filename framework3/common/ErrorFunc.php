...
//                ErrorFunc::catchAfter($e);
        ...
        if (!defined('TEST')) {
            ...
            $str .= sprintf('%s<br/>%s%s<br/>%s%s(%s)<br/>'
                            ,$ex_name
                            ,self::E_DELIMITER
                            ,$ex_message
                            ,self::E_DELIMITER
...
//            self::catchAfter($e);
    ...
    protected function funcByCode(\Throwable $e)
    {
        if (!defined('DBG')) {
            ...
            } elseif (!strcmp(get_class($e), "PDOException")) { # for database.
                self::case_PDO($e);
        ...
        try { # case of exception code that do not defined at this framework.
...
//      self::logWrite($e);
    ...
    protected static function logWrite(\Throwable $e)
    {
        ...
        error_log(F_util::hs($msg), 0); # ログを出力.
    }


    /**
     * エラー情報（production mode）を表示する.
     *
