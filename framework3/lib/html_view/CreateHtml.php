...
abstract class CreateHtml {
    ...
    public function __construct(string $temp_name, htmlDts $dt)
    {
        if (is_file(self::TEMPFILE_DIR .$temp_name.'.php')) {
			...
			try {
				 $msg = 'can\'t read file: ' . $temp_name . '.php';
...
//            ErrorFunc::catchAfter($e);
     ...
     */
    public function getCreatehtmls(CreateTagStr $obj): array
    {
        $html_strs = $obj->create_htmlstr();
     ...
     */
    public function factory_createTagStr (array $dts=null) :CreateTagStr
    {
        if (empty($dts)) {
